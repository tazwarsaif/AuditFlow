...
const checkPassword = () => {
  if(password.value != null && password.value !== '' && password2.value != null && password2.value !== ''){
...
const registerUser = () => {
  if (password.value != null && password.value !== '' && password2.value != null && password2.value !== '' && checkPassword()){
    const value = {
      ...
      "auditor_id": route.params.id
    }

    axios.post('http://127.0.0.1:8000/auth/register/', value)
      .then(res => {
        if(res.status == 201){
...
<template>
  <DefaultLayout>
    ...
    <DefaultAuthCard title="Register">
      <form>
        <div class="mb-4">
        ...
        <div class="mb-4">
          ...
          <p class="text-rose-600" v-if="!checkPassword()">Passwords do not match</p>
        </div>
