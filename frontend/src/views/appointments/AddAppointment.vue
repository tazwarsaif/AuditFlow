...
const pageTitle = ref('Add Appointment')
...
onMounted(async () => {
    try{
        const companyApi = await axios.get('http://127.0.0.1:8000/company/');
        const auditorApi = await axios.get('http://127.0.0.1:8000/auditors/');

        const responses = await Promise.all([companyApi, auditorApi]);
        data.value[0] = responses[0].data
        data.value[1] = responses[1].data
    }catch (error) {
...
const submit = async () => {
  await axios.post("http://127.0.0.1:8000/add-appointment/", {
        "company": company.value,
        "assigned_to": auditor.value,
        "status": status.value,
  ...
  <DefaultLayout>
    ...
    <DefaultAuthCard title="Add Appointment">
      ...
      <form>
        <div class="mb-4">
          ...
          <div class="relative z-20 bg-white dark:bg-form-input">
            ...
            >
              ...
              <option v-for="c in data[0]" :key="c.id" :value="c.id">{{ c.name }}</option>
            </select>
        ...
        <div class="mb-4">
          <label class="mb-2.5 block font-medium text-black dark:text-white">Audtor</label>
          <div class="relative z-20 bg-white dark:bg-form-input">
            <span class="absolute top-1/2 left-4 z-30 -translate-y-1/2">
              <svg
                width="20"
            ...
            <select
              v-model="auditor"
              :class="{ 'text-black dark:text-white': auditor }"
              class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-12 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input"
            >
              <option value="null" disabled selected>Select Auditor</option>
              <option v-for="c in data[1]" :key="c.id" :value="c.id">{{ c.first_name }} {{ c.last_name }}</option>
            </select>
            <span class="absolute top-1/2 right-4 z-10 -translate-y-1/2">
              <svg
                width="24"
        ...
        <div class="mb-4">
          <label class="mb-2.5 block font-medium text-black dark:text-white">Start Time</label>
          <div class="relative">
            <input
              type="datetime-local"
        ...
        <div class="mb-4">
          <label class="mb-2.5 block font-medium text-black dark:text-white">End Time</label>
          <div class="relative">
            <input
              type="datetime-local"
        ...
        <div class="mb-4">
          ...
          <div class="relative z-20 bg-white dark:bg-form-input">
            ...
            <select
              v-model="status"
              :class="{ 'text-black dark:text-white': status }"
              class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-12 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input"
            >
              <option v-for="(s, idx) in statuses" :key="idx" :value="s">{{ s }}</option>
            </select>
            <span class="absolute top-1/2 right-4 z-10 -translate-y-1/2">
              <svg
                width="24"
