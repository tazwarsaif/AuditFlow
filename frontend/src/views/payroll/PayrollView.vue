...
onMounted(() =>{
    axios.get('http://127.0.0.1:8000/payroll/', {
        "headers": {
            "Authorization": `Token ${localStorage.getItem('token')}`
        }
    }).then(response => {
...
const deleteItem = (id) => {
    axios.delete(`http://127.0.0.1:8000/payroll-del/${id}/`).then(res=> {
        location.reload()
    }).catch(err => console.log(err))
}
...
<template>
  <DefaultLayout>
    ...
    <!-- Breadcrumb End -->
    <div class="mb-5">
        <RouterLink class="w-full cursor-pointer rounded-lg border border-primary bg-primary p-4 font-medium text-white transition hover:bg-opacity-90" :to="{name: 'payrolls-add'}">Add Payroll Record</RouterLink>
    </div>
    ...
    >
        ...
        <table class="w-full table-auto">
            ...
            <tr class="bg-gray-2 text-left dark:bg-meta-4">
                ...
                <th class="min-w-[120px] py-4 px-4 font-medium text-black dark:text-white">Paid At</th>
                <th class="min-w-[120px] py-4 px-4 font-medium text-black dark:text-white"></th>
                </tr>
            </thead>
            <tbody>
            <tr v-for="(item, index) in payroll" :key="index">
                <td class="py-5 px-4 pl-9 xl:pl-11">
                <h5 class="font-medium text-black dark:text-white">{{ item.auditor_name }}</h5>
                </td>
                <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ item.salary }}</p>
                </td>
                <td class="py-5 px-4">
                <p class="text-black dark:text-white">{{ formatDate(item.paid_at) }}</p>
                </td>
                <td class="py-5 px-4">
                    ...
                    <div v-if="showModal" class="overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none justify-center items-center flex">
                        <div class="relative w-auto my-6 mx-auto max-w-sm">
                            ...
                            <div class="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
                                ...
                                Delete Payroll Record for {{ selectedItem.auditor_name }}?
                                </h3>
