...
const contract_expiration = ref(null)
const route = useRoute()
onMounted(async ()=> {
  await axios.get(`http://127.0.0.1:8000/company/edit/${route.params.id}`)
  .then(res=>{
    console.log(res.data)
    name.value = res.data.name;
    phone_number.value = res.data.phone;
    email.value = res.data.email;
    contract_expiration.value = res.data.contract_expiration;
  })
  .catch(err=>{
    console.log(err)
  })
...
const submit = async () => {
  ...
  await axios.post(`http://127.0.0.1:8000/company/edit/${route.params.id}`, {
    "name": name.value,
    "phone": phone_number.value,
    "email": email.value,
    "contract_expiration": contract_expiration.value
  }).then(res => {
...
<template>
  <DefaultLayout>
    ...
    <DefaultAuthCard title="Edit Company">
      <form>
        <div class="mb-4">
        ...
        <div class="mb-4">
          <label class="mb-2.5 block font-medium text-black dark:text-white">Contact Number</label>
          <div class="relative">
            <input
              type="text"
        ...
        <div class="mb-4">
          <label class="mb-2.5 block font-medium text-black dark:text-white">Contract Expiration</label>
          <div class="relative">
            <input
              type="date"
              placeholder="Contract expiration date"
              class="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary text-black dark:text-white"
              v-model="contract_expiration"
            />
